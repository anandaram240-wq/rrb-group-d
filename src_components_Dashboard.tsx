...
import { CheckCircle2, Flame, Target, Trophy, CalendarDays, Plus, Trash2, Lock, BookOpen, Zap, BarChart3 } from 'lucide-react';
...
export function Dashboard() {
  ...
  const [goals, setGoals] = useState<Goal[]>(() => {
    ...
    return [
      { id: '1', text: 'Solve 20 Math PYQs', completed: true },
      { id: '2', text: 'Review Weak Topics', completed: true },
      { id: '3', text: 'Take 1 Sectional Mock', completed: false }
    ];
  });
  ...
  const streakDays = Array.from({ length: 30 }).map((_, i) => {
    ...
    const intensity = i > 20 ? Math.floor(Math.random() * 4) : 0;
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div>
          <p className="text-on-surface-variant text-sm font-medium mb-1">Welcome back, Scholar</p>
        ...
        <div className="lg:col-span-1 space-y-8">
          ...
          <div className="bg-surface-container-lowest rounded-xl p-6 shadow-sm">
            ...
            <div className="space-y-3">
              {goals.length === 0 ? (
                <p className="text-sm text-on-surface-variant text-center py-4 italic">No goals set for today.</p>
              ) : (
                goals.map(goal => (
                  <div
                    key={goal.id}
