...
import { BookOpen, ChevronRight, CheckCircle2, XCircle, Eye, EyeOff, Search, BarChart3, Zap, Filter } from 'lucide-react';
...
export function PracticeEngine() {
  ...
  const syllabus = useMemo(() => {
    ...
    allQuestions.forEach(q => {
      ...
      if (!map[q.subject][q.topic].includes(q.sub_topic)) {
        map[q.subject][q.topic].push(q.sub_topic);
      }
    });
    return map;
  }, [allQuestions]);
  ...
  const subTopics = (selectedSubject && selectedTopic !== 'all') 
    ? syllabus[selectedSubject]?.[selectedTopic] || [] 
    : [];
  ...
  const filteredQuestions = useMemo(() => {
    ...
    if (selectedSubTopic !== 'all') qs = qs.filter(q => q.sub_topic === selectedSubTopic);
    ...
    return qs;
  }, [allQuestions, selectedSubject, selectedTopic, selectedSubTopic, difficultyFilter, searchQuery]);
      ...
      <div className="space-y-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-on-surface-variant">
                {selectedSubject} {selectedTopic !== 'all' ? `› ${selectedTopic}` : ''} {selectedSubTopic !== 'all' ? `› ${selectedSubTopic}` : ''}
              </p>
            </div>
          </div>
        ...
        <div className="bg-surface-container-lowest rounded-2xl shadow-sm border border-surface-container-high overflow-hidden">
          <div className="p-6 border-b border-surface-container bg-surface-container-low/30 flex justify-between items-center">
            ...
            <span className="text-xs font-medium text-on-surface-variant">{currentQ.topic} › {currentQ.sub_topic}</span>
          </div>

          <div className="p-8">
            <p className="text-lg font-medium text-on-surface mb-8 leading-relaxed">{currentQ.question}</p>
              ...
              {currentQ.options.map((opt, idx) => {
                ...
                return (
                  ...
                  >
                    ...
                    <span className="font-medium text-on-surface flex-1">{opt}</span>
              ...
              <div className="mt-6">
                ...
                {showSolution && currentQ.solution && (
                  <div className="mt-3 p-4 bg-primary/5 rounded-xl border border-primary/10">
                    <p className="text-sm text-on-surface leading-relaxed">{currentQ.solution}</p>
                  </div>
                )}
              </div>
            )}
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div>
          ...
          <p className="text-on-surface-variant text-sm mt-2">
            {allQuestions.length.toLocaleString()} Real PYQs • Subject → Topic → Sub-topic drill-down
          </p>
        </div>
        <div className="flex items-center gap-3">
        ...
        <div className="lg:col-span-1 space-y-4">
          <div className="bg-surface-container-lowest rounded-xl p-5 shadow-sm border border-surface-container-high">
            ...
            {selectedTopic !== 'all' && subTopics.length > 0 && (
              <div className="mb-4">
                <label className="text-[10px] font-bold text-on-surface-variant uppercase mb-2 block">Sub-Topic</label>
                <select
                  className="w-full bg-surface-container p-2 rounded-lg text-sm border-none focus:ring-2 focus:ring-primary outline-none"
                ...
                >
                  ...
                  {subTopics.map(st => {
                    const count = allQuestions.filter(q => q.subject === selectedSubject && q.topic === selectedTopic && q.sub_topic === st).length;
        ...
        <div className="lg:col-span-3">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {topics.map(topic => {
              ...
              const subTopicsList = syllabus[selectedSubject]?.[topic] || [];
              ...
              return (
                ...
                >
                  ...
                  <p className="text-[10px] text-on-surface-variant mb-3">
                    {subTopicsList.length} sub-topic{subTopicsList.length !== 1 ? 's' : ''}
                  </p>
